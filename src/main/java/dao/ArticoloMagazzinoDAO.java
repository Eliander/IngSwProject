...
public class ArticoloMagazzinoDAO {
    ...
    private final String SELECTBYCODICEIN = "SELECT * FROM ARTICOLOMAGAZZINO WHERE CODICEINGRESSO = ?";
    private final String SELECTBYCODICEOUT = "SELECT * FROM ARTICOLOMAGAZZINO WHERE CODICEUSCITA = ?";
    ...
    public ArticoloMagazzino getArticoliMagazzinoByCodice(String codice) {
    ...
    private ArticoloMagazzino getArtMagazzino(Articolo articolo, ResultSet resultset) {
        ArticoloMagazzino artMagazzino = null;
        try {
            artMagazzino = new ArticoloMagazzino(articolo, resultset.getString("codice"), new Date((resultset.getDate("dataProduzione")).getTime()), new Posizione(
